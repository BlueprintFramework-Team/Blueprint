...
int loadPngFromMemory(unsigned char* data, size_t dataLength, int* width, int* height) {
    png_image img;
    memset(&img, 0, sizeof(img));
    img.version = PNG_IMAGE_VERSION;

    if (png_image_begin_read_from_memory(&img, data, dataLength) != 0) {
        img.format = PNG_FORMAT_RGBA;
        png_uint_32 row_stride = PNG_IMAGE_ROW_STRIDE(img);
        png_bytep buffer = (png_bytep)malloc(PNG_IMAGE_BUFFER_SIZE(img, row_stride));
