...
class Sprite {
    ...
    function prepareShaderVars(anchorX:Float, anchorY:Float) {
        ...
        transform.translate([position.x + Math.abs(width) * anchorX, position.y + Math.abs(height) * -anchorY, 0]);
        ...
        Glad.uniform4f(Glad.getUniformLocation(shader.ID, "sourceRect"),
            ...
            (sourceRect.x + (width / scale.x)) / texture.width,
            (sourceRect.y + (height / scale.y)) / texture.height,
        );
    }

    function offScreen() {
        return (position.x - width * anchor.x < 0 || position.x + width * (1 - anchor.x) > Game.window.width)
            || (position.y - height * anchor.y < 0 || position.y + height * (1 - anchor.y) > Game.window.height);
    }
